...
const SidebarContent = ({ onClose, ...rest }: SidebarProps) => {
  ...
  const tenantRoles = ['administrator', 'safety-manager', 'risk-analyst', 'compliance-officer'];
  const isTenantUser = tenantRoles.some((role) => session?.user?.roles?.includes(role));
  const { data, error, isLoading } = useSWR<AirlineInterface[]>(
    () => (isTenantUser && session?.user?.tenantId ? `/airlines` : null),
    () => getAirlines({ tenant_id: session?.user?.tenantId }),
  );
  loading = isLoading;
  ...
  const LinkItems: Array<LinkItemProps> = [
    ...
    {
      name: 'Airline',
      path: isTenantUser ? `/airlines/view/${data?.[0]?.id}` : '/airlines',
      entity: 'airline',
